...
export async function POST(request: NextRequest) {
    try {
        ...
        const { user_id, name, stage_name, genre, imgUrl } = await request.json();

        // Validate the input
        if (!user_id || !stage_name || !genre) {
            return new NextResponse(
        ...
        const artistDetails = await db.client.collection("artists").create({
            user_id,
            stage_name,
            genre
        });
        ...
        return NextResponse.json({ success: true, artistDetails, userUpdate });
    } catch (err: any) {
